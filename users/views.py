    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            wallet = Wallet.objects.create(
                user=user, btc_balance=btc_balance, money_balance=money_balance
            )
            wallet.save()
            messages.success(
                request,
                f"Your account has been created with {wallet.btc_balance} bitcoins and {wallet.money_balance}$! You are now able to log in",
            )
